...
## Project Objective
- Develop a model with >97% accuracy, where at least 95% of predictions are within 10% of the actual value
- Deploy on the cloud so underwriters can access it from anywhere
...
Training a single model on the full dataset hurt accuracy. Younger and older age groups behaved very differently.

**Solution:** split into two separate models, one for age < 25 
and one for the rest, each with its own scaler.

**2. The risk score was a derived feature that users can't know:**
...
- Training inconsistencies between the notebook and production 
  are silent and dangerous. Always verify your artefacts
